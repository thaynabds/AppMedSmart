# 💊 MedSmart

**Seu assistente inteligente para gestão de saúde e adesão medicamentosa.** 🏥✨

MedSmart é um aplicativo mobile desenvolvido para ajudar pacientes a organizarem suas medicações, receberem alertas em tempo real e acompanharem sua adesão ao tratamento de forma simples e segura. 📱💜

![Capa do Projeto](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Med%20Smartt.png)

---

## 🎯 Sobre o Projeto

O **MedSmart** foi criado para reduzir erros na administração de medicamentos 💊 e aumentar a autonomia de pacientes com rotinas de tratamento complexas. Com design intuitivo 🎨 e foco em acessibilidade ♿, o aplicativo centraliza todas as informações importantes da saúde em um só lugar. 🌟

**Desenvolvido por:** Aluno do curso de **Análise e Desenvolvimento de Sistemas** 💻 da **Faculdade Senac Recife-PE** 🎓, 2025

**Status do Projeto:** 🚧 Protótipo de Alta Fidelidade no Figma

![Solução Med Smart](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Solução.png)

---

## ✨ Funcionalidades

| Funcionalidade | Descrição |
|----------------|-----------|
| 💊 **Gerenciamento de Medicamentos** | Cadastro completo com nome, dosagem, horários e observações |
| 🔔 **Alertas Inteligentes** | Notificações push para não esquecer nenhuma dose |
| 📊 **Histórico de Adesão** | Registro detalhado das doses tomadas e esquecidas |
| ❤️ **Monitoramento de Sinais Vitais** | Acompanhamento de pressão, glicemia, peso, etc. |
| 📅 **Agenda Médica** | Organização de consultas e exames |
| 🎨 **Interface Acessível** | Design clean e intuitivo ♿ |
| 🎙️ **Assistente por Voz** | Para melhor acessibilidade |
| 👥 **Perfil de Acompanhantes** | Para familiares gerenciarem a saúde de pacientes |

---

## 🎨 Protótipo do App

### 🏠 Tela Inicial
![Tela Inicial](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214072.jpg)

### 📋 Planos do Protótipo
![Planos](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214314.jpg)

### 📱 Telas do App
![Telas do App](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214737.jpg)

### 🖼️ Visão Geral do Protótipo
![Protótipo](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Protótipo.png)

**[🔗 Acessar Protótipo Interativo no Figma](https://www.figma.com/make/MsNZkHpprtDGEdfiXkXYav/App-MedSmart-2025-Protótipo-?fullscreen=1)** ✨

---

## 🌍 O Impacto ESG 🌱

### 🌿 **E – Environmental (Ambiental)**
- ♻️ Reduz o desperdício de medicamentos
- 💧 Evita uso incorreto e consultas extras, poupando recursos hospitalares

### 👥 **S – Social (Social)**
- 📱 Promove inclusão digital e alfabetização em saúde 📚
- 👴 Ajuda idosos e pessoas com baixa escolaridade
- ❤️ Melhora a adesão ao tratamento e qualidade de vida 🌟

### ⚖️ **G – Governance (Governança)**
- 🔒 Garante ética, transparência e proteção de dados (LGPD) 🛡️
- 🏥 Facilita o planejamento da saúde pública ao reduzir reincidência de doenças e erros médicos

---

## 👥 Usuários/Público Alvo

| Público | Necessidade |
|---------|-------------|
| 🚶 **Pacientes com dificuldades** | Baixa escolaridade, analfabetismo funcional, dificuldade em seguir prescrição médica |
| 👴 **Idosos** | Dificuldade de ler ou entender bulas 📖 |
| 👨‍👩‍👧 **Acompanhantes** | Precisam de todos os dados clínicos em um só lugar + lembretes de medicação ⏰ |
| 💪 **Profissionais de saúde** | Farmácias 🏥, academias 🏋️, personal trainers que desejam vender produtos/serviços |
| 🏥 **Clínicas e Hospitais** | Gestão de múltiplos pacientes, prescrições digitais 💊, alertas de alergias ⚠️, relatórios clínicos 📊 |

---

## 💼 Modelo de Negócio

![Modelo de Negócio](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Modelo%20de%20Negócios.png)

### 🆓 **Plano Med Smart Basic**
- Versão gratuita com funções básicas ✨

### 💜 **Plano Med Smart**
- Recursos avançados: 🎙️ assistente por voz, 🔔 lembrete de medicações, 📊 histórico médico, 👥 função Acompanhantes

### 🏥 **Plano MedSmart Pro**
- Recursos para hospitais, clínicas e médicos gerenciarem pacientes e fazerem prescrições 💊

---

## 📊 Mercado 📈

### **Saúde é Prioridade!** 🏥❤️

| Dado | Informação |
|------|------------|
| 🇧🇷 **Usuários SUS** | +150 milhões de brasileiros — muitos com dificuldades de compreensão médica |
| 🚀 **Crescimento HealthTech** | Mercado global cresce **18% ao ano** 📊 |
| 🤝 **Potencial de Parcerias** | Hospitais 🏥, academias 🏋️, farmácias populares 💊 e planos de saúde |
| ⭐ **Diferencial** | Ajuda prestadores de serviços médicos a chegar ao paciente de forma efetiva com propagandas baseadas em pesquisas 🔍 |

---

## 🛠️ Tecnologias Utilizadas

| Tecnologia | Uso |
|------------|-----|
| 🐍 **Python** | Linguagem principal |
| 🔧 **Flet** | Framework para apps multiplataforma |
| 🎨 **Figma** | Design e prototipagem |
| 🗄️ **Firebase/SQLite** | Planejamento para futuras versões |

---

## 🚀 Como Executar o Projeto

### 📋 Pré-requisitos
- Python 3.10 ou superior instalado ✅

### 📝 Passo a passo

```bash
# 1. Clone o repositório
git clone https://github.com/thaynabds/medsmart.git
cd medsmart

# 2. Crie um ambiente virtual (recomendado)
python -m venv venv
venv\Scripts\activate # Windows
# source venv/bin/activate # Linux/Mac

# 3. Instale as dependências
pip install flet

# 4. Execute o aplicativo
python main.py
```

---

## 👥 Equipe ESG Squad - Grupo 4 | Senac 🎓

| Nome | Função |
|------|--------|
| Débora Rafaelle Sales de Andrade | 👥 Member |
| Kayo Fernando Aragão Tavares | 📝 **Documentação** |
| Mario Souza de Moura Júnior | 📝 **Documentação** |
| Patrícia Vitória Lima de Oliveira | 🎨 **Development & Product Design** |
| **Thayná Batista da Silva** | 🧑‍💼 **Scrum Master & Analista de Requisitos**, 🔍 **Pesquisadora**, 🎨 **Product Designer** |
| Thiago Vitorino de Melo Barbosa | 📝 **Documentação** |

![Equipe ESG Squad](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Squad.png)

---

## 📄 Licença

**Copyright © 2025, Squad 4 Med Smart; Thayná Batista da Silva — App MedSmart. Todos os direitos reservados.** 🔒

---

## 📞 Contato 📬

Quer saber mais sobre o projeto ou entrar em contato? 🤝

<div align="center">

  <a href="https://br.linkedin.com/in/thaynabds" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  
  <a href="https://www.instagram.com/thaynabdstec/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>

</div>

📧 **E-mail:** [thaynabdstec@gmail.com](mailto:thaynabdstec@gmail.com) ✉️

📱 **Telefone:** +55 (81) 97912-6121 📞

---

<div align="center">

### 👤 Thayná Batista da Silva
**Scrum Master • Product Designer • Analista de Requisitos** 💼

Faculdade Senac Recife-PE | Análise e Desenvolvimento de Sistemas 🎓

![Cartão TEC Thayná](https://github.com/thaynabds/AppMedSmart/blob/main/CapaCartão%20ThaynáBDSTEC.png)

</div>

---

# 💊 MedSmart (English Version) 🌐

**Your intelligent assistant for health management and medication adherence.** 🏥✨

MedSmart is a mobile application developed to help patients organize their medications 💊, receive real-time alerts 🔔, and track their treatment adherence in a simple and secure way. 📱💜

![Project Cover](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Med%20Smartt.png)

---

## 🎯 About the Project

**MedSmart** was created to reduce medication administration errors ❌ and increase autonomy for patients with complex treatment routines. With intuitive design 🎨 and accessibility focus ♿, the app centralizes all important health information in one place. 🌟

**Developed by:** Student of the **Systems Analysis and Development** course 💻 at **Senac Recife-PE Faculty** 🎓, 2025

**Project Status:** 🚧 High-Fidelity Prototype in Figma

![Med Smart Solution](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Solução.png)

---

## ✨ Features

| Feature | Description |
|---------|-------------|
| 💊 **Medication Management** | Complete registration with name, dosage, schedules, and notes |
| 🔔 **Smart Alerts** | Push notifications to never miss a dose |
| 📊 **Adherence History** | Detailed record of taken and missed doses |
| ❤️ **Vital Signs Monitoring** | Track blood pressure, blood glucose, weight, etc. |
| 📅 **Medical Agenda** | Organize appointments and exams |
| 🎨 **Accessible Interface** | Clean and intuitive design ♿ |
| 🎙️ **Voice Assistant** | For better accessibility |
| 👥 **Caregiver Profile** | For family members to manage patient health |

---

## 🎨 App Prototype

### 🏠 Home Screen
![Home Screen](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214072.jpg)

### 📋 Prototype Plans
![Plans](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214314.jpg)

### 📱 App Screens
![App Screens](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/1770759214737.jpg)

### 🖼️ Prototype Overview
![Prototype](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Protótipo.png)

**[🔗 Access Interactive Prototype on Figma](https://www.figma.com/make/MsNZkHpprtDGEdfiXkXYav/App-MedSmart-2025-Protótipo-?fullscreen=1)** ✨

---

## 🌍 ESG Impact 🌱

### 🌿 **E – Environmental**
- ♻️ Reduces medication waste
- 💧 Prevents incorrect use and extra consultations, saving hospital resources

### 👥 **S – Social**
- 📱 Promotes digital inclusion and health literacy 📚
- 👴 Helps elderly people and those with low education
- ❤️ Improves treatment adherence and quality of life 🌟

### ⚖️ **G – Governance**
- 🔒 Ensures ethics, transparency, and data protection (LGPD/GDPR) 🛡️
- 🏥 Facilitates public health planning by reducing disease recurrence and medical errors

---

## 👥 Target Users

| Audience | Need |
|----------|------|
| 🚶 **Patients with difficulties** | Low education, functional illiteracy, difficulty following medical prescriptions |
| 👴 **Elderly** | Difficulty reading or understanding package inserts 📖 |
| 👨‍👩‍👧 **Caregivers** | Need all clinical data in one place + medication reminders ⏰ |
| 💪 **Health professionals** | Pharmacies 🏥, gyms 🏋️, personal trainers wanting to sell products/services |
| 🏥 **Clinics and Hospitals** | Multi-patient management, digital prescriptions 💊, allergy alerts ⚠️, clinical reports 📊 |

---

## 💼 Business Model

![Business Model](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Modelo%20de%20Negócios.png)

### 🆓 **Med Smart Basic Plan**
- Free version with basic features ✨

### 💜 **Med Smart Plan**
- Advanced features: 🎙️ voice assistant, 🔔 medication reminders, 📊 medical history, 👥 Caregiver feature

### 🏥 **MedSmart Pro Plan**
- Features for hospitals, clinics, and doctors to manage patients and make prescriptions 💊

---

## 📊 Market 📈

### **Health is a Priority!** 🏥❤️

| Data | Information |
|------|-------------|
| 🇧🇷 **SUS Users** | +150 million Brazilians — many with medical comprehension difficulties |
| 🚀 **HealthTech Growth** | Global market grows **18% per year** 📊 |
| 🤝 **Partnership Potential** | Hospitals 🏥, gyms 🏋️, popular pharmacies 💊, and health plans |
| ⭐ **Differentiator** | Helps medical service providers reach patients effectively with ads based on medication and procedure searches 🔍 |

---

## 🛠️ Technologies Used

| Technology | Purpose |
|------------|---------|
| 🐍 **Python** | Main programming language |
| 🔧 **Flet** | Cross-platform app framework |
| 🎨 **Figma** | Design and prototyping |
| 🗄️ **Firebase/SQLite** | Future versions planned |

---

## 🚀 How to Run the Project

### 📋 Prerequisites
- Python 3.10 or higher installed ✅

### 📝 Step by step

```bash
# 1. Clone the repository
git clone https://github.com/thaynabds/medsmart.git
cd medsmart

# 2. Create a virtual environment (recommended)
python -m venv venv
venv\Scripts\activate # Windows
# source venv/bin/activate # Linux/Mac

# 3. Install dependencies
pip install flet

# 4. Run the application
python main.py
```

---

## 👥 ESG Squad Team - Group 4 | Senac 🎓

| Name | Role |
|------|------|
| Débora Rafaelle Sales de Andrade | 👥 Member |
| Kayo Fernando Aragão Tavares | 📝 **Documentation** |
| Mario Souza de Moura Júnior | 📝 **Documentation** |
| Patrícia Vitória Lima de Oliveira | 🎨 **Development & Product Design** |
| **Thayná Batista da Silva** | 🧑‍💼 **Scrum Master & Requirements Analyst**, 🔍 **Researcher**, 🎨 **Product Designer** |
| Thiago Vitorino de Melo Barbosa | 📝 **Documentation** |

![ESG Squad Team](https://raw.githubusercontent.com/thaynabds/AppMedSmart/refs/heads/main/Squad.png)

---

## 📄 License

**Copyright © 2025, Squad 4 Med Smart; Thayná Batista da Silva — App MedSmart. All rights reserved.** 🔒

---

## 📞 Contact 📬

Want to know more about the project or get in touch? 🤝

<div align="center">

  <a href="https://br.linkedin.com/in/thaynabds" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  
  <a href="https://www.instagram.com/thaynabdstec/" target="_blank">
    <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
  </a>

</div>

📧 **Email:** [thaynabdstec@gmail.com](mailto:thaynabdstec@gmail.com) ✉️

📱 **Phone:** +55 (81) 97912-6121 📞

---

<div align="center">

### 👤 Thayná Batista da Silva
**Scrum Master • Product Designer • Requirements Analyst** 💼

Senac Recife-PE Faculty | Systems Analysis and Development 🎓

![TEC Card Thayná](https://github.com/thaynabds/AppMedSmart/blob/main/CapaCartão%20ThaynáBDSTEC.png)

</div>

---

<div align="center">

**Thank you for visiting! ⭐ Don't forget to star the project if you liked it!** 🌟

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://br.linkedin.com/in/thaynabds)
[![Instagram](https://img.shields.io/badge/Instagram-Follow-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/thaynabdstec/)

</div>
